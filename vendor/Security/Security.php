...
class Security
{
    ...
    public function checkParameters($table, $Data, $dataMoins)
    {
        $moins = (isset($dataMoins['id'])) ?  count($dataMoins) : count($dataMoins) + 1;
        require __DIR__ . "/../../src/BackOffice/Entities.php";

        // On compare les indices de 2 tableaux (ex: (array)$Data et (array)$produit).
        $ArrayDiff = array_diff_key($Data, ${$table});
        if (!empty($ArrayDiff))
        {
        ...
        $arrayInter = array_intersect_key($Data, ${$table});
        if (count(${$table}) - $moins == count($arrayInter))
        {
            // On va vérifier les données
            $this->valideParameters($Data, ${$table});
            return $Data;
        }
        else
        {
