...
foreach ($files as $f){
  ...
  $key = array_search("lokisalle",$arrayPath);
  $output = array_slice($arrayPath, $key);
  $fullPath= implode("/",$output);
  $str .= '{"p":"'.mb_ereg_replace('"', '\\"', "http://".$_SERVER['SERVER_NAME']."/".$fullPath).'","s":"'.$size.'","t":"'.$time.'","w":"'.$w.'","h":"'.$h.'"},';
}
